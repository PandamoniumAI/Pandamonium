...
export default function Chat() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      console.error("Error fetching messages ERROR: 558");
    }
  };
    ...
    if (!firstMessageSent) {
      ...
      try {
        ...
        if (response && response.result && response.result.response) {
          ...
          if (botResponse) {
            const parsedContent = botResponse.replace(
              /\{message\.content\}/g,
              input,
            );
  ...
  return (
    <div
      className="d-flex h-100 bg-dark text-white overflow-hidden"
      style={{ height: "100vh" }}
    >
      <Sidebar />
      <div
            ...
            <div className="d-flex justify-content-start mb-4 animate__animated animate__fadeIn">
              ...
              >
                <span className="text-muted" style={{ fontSize: "14px" }}>
                  <span className="animate-pulse">...</span> {characterName} is
                  typing...
                </span>
              </div>
            </div>
              ...
              if (userMessage !== "") {
                const checkAndSaveMessage = () => {
                  const botMessage = JSON.parse(
                    localStorage.getItem("botMessage"),
                  );
                  if (botMessage && botMessage.content) {
                    import("./Saving/Saving").then((module) => {
                      const data = {
                        id: id,
                        characterMessage: botMessage.content,
                        userMessage,
                      };
