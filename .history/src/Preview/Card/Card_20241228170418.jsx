...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-6">
  {Object.entries(CharacterCards).map(([key, character]) => (
    <div
      className="w-full max-w-sm border-2 border-gray-300 rounded-2xl shadow-lg transform transition-all duration-500 ease-in-out hover:scale-105 hover:shadow-2xl hover:rotate-2 cursor-pointer bg-transparent overflow-hidden group"
      key={key}
      onClick={() => openPopup(character)}
    >
      <div className="relative w-full h-56">
        <img
          className="w-full h-full object-cover rounded-t-2xl group-hover:opacity-80 transition-opacity duration-500"
          src={character.image}
      ...
      <div className="p-4 bg-gradient-to-r from-red-700 via-red-600 to-red-500 rounded-b-2xl">
        <h2 className="text-white text-2xl font-semibold mb-2 text-center drop-shadow-lg">{character.info.name}</h2>
        <p className="text-white text-sm opacity-90 mb-4 text-center drop-shadow-md">{character.info.preview}</p>
