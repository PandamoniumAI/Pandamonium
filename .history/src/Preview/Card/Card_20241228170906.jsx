...
import { FaComment } from "react-icons/fa"; // Import chat icon
        ...
        <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-4">
                {Object.entries(CharacterCards).map(([key, character]) => (
                    <div
                        className="w-[194px] h-[323px] border-[0.5px] border-gray-300 rounded-[15px] shadow-lg transform transition-all duration-500 ease-in-out hover:scale-105 hover:shadow-xl hover:translate-y-[-10px] cursor-pointer bg-transparent overflow-hidden group"
                        key={key}
                    ...
                    >
                        ...
                        <div className="p-4 bg-gradient-to-r from-red-700 via-red-600 to-red-500 rounded-b-[15px] flex flex-col justify-between h-full">
                            <h2 className="text-white text-sm font-semibold mb-2 text-left drop-shadow-lg">{character.info.name}</h2>
                            <p className="text-white text-xs opacity-90 mb-2 text-left drop-shadow-md">{character.info.preview}</p>
                            <div className="flex items-center justify-start text-xs text-white opacity-80 space-x-1">
