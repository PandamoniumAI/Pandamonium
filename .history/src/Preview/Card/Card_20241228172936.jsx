        ...
        <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 p-4">
                {Object.entries(CharacterCards).map(([key, character]) => (
                    <div
                        className="w-[194px] h-[323px] border-[0.5px] border-[#C62D01] rounded-[15px] shadow-lg transform transition-all duration-500 ease-in-out hover:scale-105 hover:shadow-2xl hover:translate-y-[-10px] cursor-pointer bg-transparent overflow-hidden group hover:rotate-2"
                        key={key}
                    ...
                    >
                        <div className="relative w-full h-full">
                            ...
                            {character.NSFW && (
                                <div className="absolute top-2 right-2 bg-[#FF00BFE5] text-white text-xs rounded-[5px] px-2 py-1">
                                    NSFW
                                </div>
                            )}
                        </div>
                        <div className="absolute bottom-0 w-full p-4 bg-gradient-to-r from-red-700 via-red-600 to-red-500 rounded-b-[15px] opacity-85">
                            ...
                            <p className="text-white text-xs opacity-90 mb-4 text-left drop-shadow-md">{character.info.preview}</p>
