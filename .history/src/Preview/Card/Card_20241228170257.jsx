...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-5">
  {Object.entries(CharacterCards).map(([key, character]) => (
    <div
      className="w-full max-w-xs border border-gray-300 rounded-2xl shadow-lg transform transition-all duration-500 ease-in-out hover:scale-105 hover:shadow-2xl hover:rotate-1 cursor-pointer bg-white"
      key={key}
      onClick={() => openPopup(character)}
    >
      <img
        className="w-full h-64 object-cover rounded-t-2xl filter hover:blur-sm"
        src={character.image}
        alt={character.info.name}
      />
      <div className="p-6" style={{ backgroundColor: '#C62D01' }}>
