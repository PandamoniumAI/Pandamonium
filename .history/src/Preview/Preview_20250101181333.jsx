...
export default function Preview() {
  ...
  {["Multiple Character Roleplays", "Tools for Creators", "Community-Driven No Filter"].map(
    (title, index) => (
      <div
        key={index}
        className="card p-6 bg-gradient-to-b from-[#E03201] to-[#611600] text-white shadow-xl rounded-lg relative overflow-hidden flex flex-col items-center text-center"
      >
        <div className="icon-container mb-4">
          {index === 0 ? (
            <img src="/path/to/roleplay-icon.png" alt="Roleplay Icon" className="w-16 h-16" />
          ) : index === 1 ? (
            <img src="/path/to/creator-tools-icon.png" alt="Creator Tools Icon" className="w-16 h-16" />
          ) : (
            <img src="/path/to/community-icon.png" alt="Community Icon" className="w-16 h-16" />
          )}
        </div>
