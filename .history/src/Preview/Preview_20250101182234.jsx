...
export default function Preview() {
    ...
    <div className="main">
      ...
      <div className="preview-container relative p-6">
        ...
        <div className="Buttons absolute top-[500px] flex space-x-4 mb-8">
          <button className="text-white text-xl h-[47px] w-[200px] rounded-lg shadow-2xl bg-gradient-to-r from-[#E03201] to-[#611600] transform transition-all duration-300 ease-in-out">
            <span className="relative z-10">Explore</span>
          </button>
          <button
            className="create bg-black text-white text-xl h-[47px] w-[200px] rounded-lg shadow-2xl outline outline-1 transition-all duration-300 ease-in-out relative overflow-hidden group"
            style={{
              outlineImage:
  ...
  {["Multiple Character Roleplays", "Tools for Creators", "Community-Driven No Filter"].map(
    (title, index) => (
      <div
        key={index}
        className="card relative flex flex-col justify-between w-[284px] h-[477px] rounded-[25px] border-2 border-transparent"
        style={{
          borderImage: 'linear-gradient(135deg, #BF18C2, #6B1C8F, #1D0375) 1',
        }}
      >
        <div className="content p-6 flex flex-col justify-between h-full rounded-[25px] bg-transparent">
        ...
        <div className="image-container mt-4">
          {index === 0 ? (
            <img src="/path/to/roleplay-image.png" alt="Roleplay" className="w-full rounded-[25px]" />
          ) : index === 1 ? (
            <img src="/path/to/creator-tools-image.png" alt="Creator Tools" className="w-full rounded-[25px]" />
          ) : (
            <img src="/path/to/community-image.png" alt="Community" className="w-full rounded-[25px]" />
          )}
        </div>
        {/* Apply outline with color change effect */}
        <div className="absolute inset-0 border-2 border-transparent rounded-[25px] bg-gradient-to-br from-pink-500 via-purple-600 to-blue-700 opacity-50 hover:opacity-100 transition-opacity duration-300"></div>
      </div>
    )
  )}
</div>
