...
export default function Preview() {
  ...
  {["Multiple Character Roleplays", "Tools for Creators", "Community-Driven No Filter"].map(
    (title, index) => (
      <div
        key={index}
        className="card relative overflow-hidden flex flex-col justify-between w-[284px] h-[477px] border-2 border-transparent rounded-lg"
        style={{
          borderImage: 'linear-gradient(135deg, #BF18C2, #6B1C8F, #1D0375) 1',
        }}
      >
        <div className="content p-6 bg-[#1A1A1A] flex flex-col justify-between h-full rounded-lg">
        ...
        <div className="image-container mt-4">
          {index === 0 ? (
            <img src="/path/to/roleplay-image.png" alt="Roleplay" className="w-full rounded-lg" />
          ) : index === 1 ? (
            <img src="/path/to/creator-tools-image.png" alt="Creator Tools" className="w-full rounded-lg" />
          ) : (
            <img src="/path/to/community-image.png" alt="Community" className="w-full rounded-lg" />
          )}
        </div>
      </div>
    )
