...
export default function Preview() {
  ...
  {["Multiple Character Roleplays", "Tools for Creators", "Community-Driven No Filter"].map(
    (title, index) => (
      <div
        key={index}
        className="card p-6 text-white shadow-xl rounded-lg relative overflow-hidden flex flex-col justify-between h-96 bg-black"
      >
        <div className="absolute inset-0 z-[-1] rounded-lg bg-gradient-to-br from-pink-600 via-purple-600 to-blue-900"></div>
        <div className="content p-6 bg-black rounded-lg">
        ...
        <div className="image-container mt-4">
          {index === 0 ? (
            <img src="/path/to/roleplay-image.png" alt="Roleplay" className="w-full rounded-md" />
          ) : index === 1 ? (
            <img src="/path/to/creator-tools-image.png" alt="Creator Tools" className="w-full rounded-md" />
          ) : (
            <img src="/path/to/community-image.png" alt="Community" className="w-full rounded-md" />
          )}
        </div>
      </div>
    )
