...
const chatsPath = path.join(__dirname, 'Datastore', 'JSONS', 'chats.json');

export const saveChat = async (chatData) => {
    try {
        if (!fs.existsSync(path.join(__dirname, 'Datastore', 'JSONS'))) {
            fs.mkdirSync(path.join(__dirname, 'Datastore', 'JSONS'), { recursive: true });
        }
